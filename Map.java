...
public class Map {
    ...
    private final Map<ProductType, String[]> productNames = new HashMap<>();
    private final Map<ProductType, double[]> priceRanges = new HashMap<>();
    ...
    private void initializeProductData() {
        productNames.put(ProductType.VEGETABLE, new String[]{"Cabbage", "Lettuce", "Broccoli", "Carrot", "Tomato"});
        ...
        productNames.put(ProductType.EGGS, new String[]{"Brown Eggs", "Quail Eggs", "Free-Range"});
        ...
        priceRanges.put(ProductType.EGGS, new double[]{60, 150});
    ...
    private void initializeMap() {
        ...
        createTableRow(4, 7, 17, "GF, Aisle 3, Table ", ProductType.VEGETABLE);
        createTableRow(4, 7, 19, "GF, Aisle 3, Table ", ProductType.BREAD);
        createTableRow(4, 7, 21, "GF, Aisle 3, Table ", ProductType.EGGS);
    ...
    private void createChilledRow(int start, int end, int col, String base, ProductType type) {
    ...
    private ProductType getProductTypeForAddress(String address) {
        ...
        if (address.contains("Table 4")) return ProductType.EGGS;
                ...
                if (r == shopper.getRow() && c == shopper.getCol()) System.out.print(switch (shopper.getFacing()) {
                    ...
                    char sym = switch (t.getType()) {
                        ...
                        case DISPLAY -> switch (((Display) t.getAmenity()).getDisplayType()) {
                            case TABLE -> 'T'; case CHILLED_COUNTER -> 'C'; case REFRIGERATOR -> 'R'; case SHELF -> 'S';
                        };
                        case SERVICE -> {
    ...
    private void interactWithDisplay(Display d) {
        ...
        if (choice == 1) {
            for (int tier = 0; tier < d.getDisplayType().getNumTiers(); tier++) {
                ...
                for (Product p : d.getTiers().get(tier)) System.out.println(p.getSerialCode() + " " + p.getName() + " ₱" + p.getPrice());
            }
            int tier = d.getDisplayType().getNumTiers() > 1 ? getInt("Tier: ") : 0;
            String serial = getString("Serial: ");
            Product p = d.removeProduct(serial, tier);
            if (p == null) { System.out.println("Not found."); return; }
            if (!shopper.canTakeProduct(p)) { d.addProduct(p, tier); System.out.println("Restricted."); return; }
            if (!shopper.addProduct(p)) { d.addProduct(p, tier); System.out.println("Full."); return; }
            System.out.println("Taken.");
        } else if (choice == 2) {
            List<Product> prods = shopper.getChosenProducts();
            if (prods.isEmpty()) { System.out.println("None."); return; }
            for (Product p : prods) System.out.println(p.getSerialCode() + " " + p.getName() + " ₱" + p.getPrice());
            String serial = getString("Serial: ");
            Product p = shopper.removeProduct(serial);
            if (p == null) { System.out.println("Not found."); return; }
            if (!p.getType().getDisplayType().equals(d.getDisplayType())) { shopper.addProduct(p); System.out.println("Wrong display."); return; }
            int tier = d.getDisplayType().getNumTiers() > 1 ? getInt("Tier: ") : 0;
            if (!d.addProduct(p, tier)) { shopper.addProduct(p); System.out.println("Full."); return; }
    ...
    private int getInt() { return getInt(""); }
    private int getInt(String prompt) { System.out.print(prompt); int i = scanner.nextInt(); scanner.nextLine(); return i; }
    private String getString(String prompt) { System.out.print(prompt); return scanner.nextLine(); }
}